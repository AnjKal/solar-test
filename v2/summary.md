...
| High blackout risk shaping (`blackout_probability > 0.6`) | Grid reliability is poor / outage likely soon. Operators should proactively transition to island mode to avoid instability and protect loads. | Under high risk and not maintenance: penalize not islanding (**−10**). If islanding: base bonus **+2**, plus correct mode bonus **+2** (else **−2**) based on expected outage duration. |
